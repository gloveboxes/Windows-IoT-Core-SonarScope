...
std::map<alljoyn_busobject, WeakReference*> SonarProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> SonarProducer::SourceInterfaces;

SonarProducer::SonarProducer(AllJoynBusAttachment^ busAttachment)
...
void SonarProducer::UnregisterFromBus()
{
...
bool SonarProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void SonarProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<SonarProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<SonarProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<SonarProducer>
    };
...
void SonarProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void SonarProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void SonarProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void SonarProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<SonarProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
void SonarProducer::CallStopHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    ...
    SonarProducer^ producer = source->second->Resolve<SonarProducer>();
    if (producer->Service != nullptr)
    {
        ...
        SonarStopResult^ result = create_task(producer->Service->StopAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
void SonarProducer::CallStartHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    auto source = SourceObjects.find(busObject);
    if (source == SourceObjects.end())
    {
    ...
    if (producer->Service != nullptr)
    {
        ...
        SonarStartResult^ result = create_task(producer->Service->StartAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
void SonarProducer::CallSpeechHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    auto source = SourceObjects.find(busObject);
    if (source == SourceObjects.end())
    {
    ...
    if (producer->Service != nullptr)
    {
        ...
        SonarSpeechResult^ result = create_task(producer->Service->SpeechAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
QStatus SonarProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus SonarProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus SonarProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
...
QStatus SonarProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(propertyName);
    UNREFERENCED_PARAMETER(value);
    UNREFERENCED_PARAMETER(interfaceName);
...
void SonarProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_SonarIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<SonarProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "org.alljoyn.example.Sonar");
    ...
    result = AddMethodHandler(
        interfaceDescription, 
        "Start", 
        [](alljoyn_busobject busObject, const alljoyn_interfacedescription_member* member, alljoyn_message message) { UNREFERENCED_PARAMETER(member); CallStartHandler(busObject, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AddMethodHandler(
        interfaceDescription, 
        "Speech", 
        [](alljoyn_busobject busObject, const alljoyn_interfacedescription_member* member, alljoyn_message message) { UNREFERENCED_PARAMETER(member); CallSpeechHandler(busObject, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &SonarProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void SonarProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void SonarProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 SonarProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR org::alljoyn::example::Sonar::c_SonarIntrospectionXml = "<interface name=\"org.alljoyn.example.Sonar\">"
"  <annotation name=\"org.alljoyn.Bus.Secure\" value=\"false\" />"
"  <description language=\"en\">Interface for controlling a Sonar Scope</description>"
"  <method name=\"Stop\">"
